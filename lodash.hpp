...
#undef max
    ...
    namespace detail
    {
        ...
        struct is_detected<std::void_t<T<Args...>>, T, Args...>
            : std::true_type
        { };
    } // namespace detail
    ...
    namespace helper
    {
        ...
        typename std::enable_if<traits::has_push_back<Container>::value, void>::type
            add_to_container(Container& container, const typename Container::value_type& value)
        {
...
//#include <map>
    ...
    {
        return map(container, [&] (auto& key)
        {
            return function(container[key], key);
        });
    }
    ...
    // invoke
    template <typename ResultContainer, typename Container, typename Function>
    typename std::enable_if<!std::is_void<ResultContainer>::value, ResultContainer>::type
        invoke(const Container& container, Function&& function)
    {
        return map(container, std::mem_fn(std::forward<Function>(function)));
    }

    template <typename ResultContainer, typename Container, typename Function>
    typename std::enable_if<std::is_void<ResultContainer>::value, void>::type
        invoke(const Container& container, Function function)
    {
    ...
    */
    ...
    T reduce(const Container& container, T init, BinaryOperation op)
    {
        return std::accumulate(std::begin(container), std::end(container), std::move(init), std::move(op));
    }

    template <typename Container, typename T = typename Container::value_type>
    T sum(const Container& container, T init = T())
    {
        return reduce(container, std::move(init), std::plus<T>());
    }
    ...
    {
        return reduce(container, init, std::max<T>);
    }
    ...
    template <typename Compared, typename Container, typename Function>
    typename Container::iterator min(const Container& container, Function function)
    {
        if (container.begin() == container.end())
        {
            return container.end();
    ...
    typename Container::iterator sorted_index(
        const Container& container, typename Container::value_type const& value)
    {
    ...
    namespace helper
    {
        ...
        template <typename ResultContainer, typename Container>
        typename std::enable_if<!HasConstIterator<typename Container::value_type>::value, void>::type
    ...
    typename std::enable_if<shallow == true, ResultContainer>::type
    ...
    template <typename ResultContainer, bool shallow, typename Container>
    typename std::enable_if<shallow == false, ResultContainer>::type
        flatten(Container const& container)
    {
    ...
    template <typename Container>
    auto& getOrCall(const Container& container, const typename Container::key_type key, typename Container::value_type(*function)(typename Container::key_type)) {
        if (!contains(container, key))
    ...
    template <typename Container>
    auto& getOrDefault(const Container& container, const typename Container::key_type key, const typename Container::value_type& value) {
        if (!contains(container, key))
